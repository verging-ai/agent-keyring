    ...
    pub fn new() -> Self {
        Self {
            meta: ConnectorMeta {
                ...
                config_path_patterns: vec!["~/.codex/config.json".to_string()],
...
impl Connector for CodexCliConnector {
    ...
    fn backup(&self) -> ConnectorResult<String> {
        ConnectorResult::err("CONNECTOR_NOT_IMPLEMENTED", "Backup not yet implemented for Codex CLI")
    }

    fn rollback(&self, _backup_path: &str) -> ConnectorResult<()> {
        ConnectorResult::err("CONNECTOR_NOT_IMPLEMENTED", "Rollback not yet implemented for Codex CLI")
    }
}
